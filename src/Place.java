...
public class Place extends JPanel implements MouseListener {
	...
	private int i;
	private ActionListener alBack, alNext;

	public Place(ImageInfo ii, int pos, int TLx, int TLy, int TRx, int TRy, int BLx, int BLy, int BRx, int BRy,
			JButton... buttons) {
		...
		alBack = new ActionListener(){
			@Override // following method is in the original superclass and you
						// are overriding it
			public void actionPerformed(ActionEvent e) {
				Driver.removeAndGoBack();
			}

		};
		alNext = new ActionListener(){
			@Override // following method is in the original superclass and you
						// are overriding it
			public void actionPerformed(ActionEvent e) {
				Driver.removeAndAdd();
		...
		if (buttons != null) {
			for (i = 0; i < buttons.length; i++) {
				...
				if(buttons[i].getText().equals("Back") || buttons[i].getText().equals("Goodbye")){
